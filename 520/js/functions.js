...
$(function () {
    // setup garden
	$loveHeart = $("#loveHeart");
    $loveHeart.height($loveHeart.width() * 0.9);
    $loveHeartW = $loveHeart.width();
    $loveHeartH = $loveHeart.height();
    $garden = $("#garden");
...
function getHeartPoint(angle) {
	var t = angle / Math.PI;
	var x = 19 * ($loveHeartW / 675) * (16 * Math.pow(Math.sin(t), 3));
	var y = - 20 * ($loveHeartH / 620) * (13 * Math.cos(t) - 5 * Math.cos(2 * t) - 2 * Math.cos(3 * t) - Math.cos(4 * t));
	return new Array(_offsetX + x, _offsetY + y);
}
