;(function (window) {
    ...
    function drawText(ctx) {
        let autoSize = CANVASWIDTH / (text.length < 7 ? 7 : text.length) + 1;
        let textSize = isPhone ? parseInt(autoSize) : 70;
    ...
    function particleText(imgData) {
        // 点坐标获取
        let pxls = [];
        for (let w = CANVASWIDTH; w > 0; w -= (isPhone ? 2 : 2)) {
            for (let h = 0; h < CANVASHEIGHT; h += (isPhone ? 1 : 3)) {
    ...
    function event() {
        BALL_LIST = document.querySelectorAll('.letter-balls');
        let iconLetters = document.querySelector('.icon-letters');
        document.addEventListener('click', function (e) {
            iconLetters.style.display = 'block';
            if (e.clientY < WINDOW_H / 2){
                textIndex --;
            }else {
                textIndex++;
            }
            textIndex > texts.length && (textIndex = texts.length);
            textIndex < 0 && (textIndex = 0);

            text = texts[textIndex];
            let curBall = BALL_LIST[textIndex % BALL_LIST.length];
            let innerBall = curBall.querySelector('span');
            let clientX = e.clientX;
            let clientY = e.clientY;
            curBall.style.top = clientY + 'px';
            innerBall.style.left = clientX + 'px';
            curBall.style.display = 'block';
            innerBall.style.display = 'block';
            curBall.clientWidth;
            innerBall.clientWidth;

            curBall.style.top = WINDOW_H - 100 + 'px';
            innerBall.style.left = WINDOW_W / 2  + 'px';
        }, false);
        BALL_LIST.forEach(function(item){
